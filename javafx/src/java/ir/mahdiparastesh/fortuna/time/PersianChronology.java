 ...
 * <p>
 * Normal years have 365 days and leap years have 366 days.
 * <p>
...
public final class PersianChronology extends AbstractChronology {
    ...
    /**
     * Checks whether parameter {@code value} is valid or not. If {@code value} is out
    ...
    void checkValidValue(long value, TemporalField field) {
        ...
        if (!JalaliUtils.isBetween(value, range(cf).getMinimum(), range(cf).getMaximum())) {
    ...
    void checkDayOfYear(int year, int dayOfYear) {
        ...
        if (!JalaliUtils.isBetween(dayOfYear, 1, maxDayOfYear)) {
